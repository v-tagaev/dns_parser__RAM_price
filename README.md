...
2) реализован запуск на Docker в Airflow с ежедневным расписанием. Airflow подключается через SSH к браузеру на Windows и производит парсинг данных в базу Postgres.
   Обойти защиты сайта с самого docker пока не получилось, защиты отслеживают сигнатуры кастрированного linux, имитация бурной деятельности человека, эмуляция экрана, скрытые режимы playwright, прокси - не помогли.
