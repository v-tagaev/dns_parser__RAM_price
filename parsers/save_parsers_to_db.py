...
# загружаем .env только если мы НЕ в Airflow -- для тестов с винды
if 'AIRFLOW_HOME' not in os.environ:
    # Пытаемся найти .env на уровень выше (если запустили из папки parsers)
    # Если не нашли, ищем в текущей папке (если запустили из корня)
...
def save_df_to_postgres(df: pd.DataFrame):
    ...
    with engine.begin() as connection:
        ...
        # -------------------------------------------------------------------------
        # Шаг 2. Загружаем цены для всех позиций
        # -------------------------------------------------------------------------
